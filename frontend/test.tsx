...
// Updated schema with optional Content field
...
const MentorDashboard = () => {
  ...
  const [selectedCourse, setSelectedCourse] = useState<ICourses | null | any>(
    null
  );
  const [updateCourses, setUpdateCourse] = useState<ICourses | null | any>(
    null
  );
  ...
  console.log("up data is ", updateCourses);
  ...
  const setinitalDaft = (data: any) => {
    Setinitaldata(data);
    setCreateOpen(true);
  };

  // Updated handleSaveChanges with validation
  const handleSaveChanges = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        // If you have an updateCourse API function:
        await updateCourse(selectedCourse._id, updateCourses);
  ...
  // Updated handleInputChange to handle different input types
          ...
          <Card className="bg-gradient-to-r from-blue-800 to-blue-700 text-white border-none shadow-lg">
            ...
            <CardContent>
              ...
              <p className="text-blue-200 text-sm mt-2">
                {
                  courses.filter((c) => c.Approved_by_admin === "approved")
                    .length
                }{" "}
                approved,{" "}
                {
                  courses.filter((c) => c.Approved_by_admin === "pending")
                    .length
                }{" "}
                pending
              </p>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        id="Category"
                        name="Category"
                        value={selectedCourse.Category?._id || ""}
                        onChange={handleInputChange}
