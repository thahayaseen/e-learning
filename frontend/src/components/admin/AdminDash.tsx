...
import { ChartSkeleton, chartTheme, StatSkeleton, TableSkeleton } from "@/services/admintheam";
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : "Cannot fetch datas"
        );
      } finally {
  ...
  useEffect(() => {
    const fetchTimeRevenue = async () => {
      ...
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : "cannot Fetch data"
        );
      } finally {
  ...
  const fetchOrders = async (page) => {
    ...
    try {
      const limit = 3;
      const ordersData: any = await ordersRes(page, limit);
      setDashboardData((prev) => ({
        ...prev,
            ...
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg shadow-lg p-6 border border-gray-700 transition-transform hover:scale-102 hover:shadow-xl">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-white">
                    {formatCurrency(dashboardData.totalRevenue * 0.1)}
                  </p>
                </div>
              </div>
            </div>
