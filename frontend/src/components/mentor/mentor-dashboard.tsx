  ...
  return (
    <div className="w-full bg-gradient-to-b mx-1 from-blue-950 to-blue-900 min-h-screen">
      <div className={`w-full p-4 ${createCourseOpen ? "blur-sm" : ""}`}>
        <div className="flex w-full justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              {isFilterOpen && (
                <CourseFilters filters={filters} setFilters={setFilters} setIsFilterOpen={setIsFilterOpen} />
              )}
            </div>
        ...
        <DashboardStats totalCourses={totalCourses} totalStudents={totalStudents} totalRevenue={totalRevenue} />
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {searchTerm && (
              <Badge className="bg-blue-700 text-white px-3 py-1">
                Search: {searchTerm}
                <button className="ml-2 hover:text-blue-200" onClick={() => setSearchTerm("")}>
                  ×
                </button>
              </Badge>
            )}
            ...
            {filters.status !== "all" && (
              <Badge className="bg-blue-700 text-white px-3 py-1">
                ...
                <button className="ml-2 hover:text-blue-200" onClick={() => setFilters({ ...filters, status: "all" })}>
                  ×
                </button>
              </Badge>
            )}
            ...
            {(searchTerm || filters.status !== "all" || filters.priceRange !== "all") && (
              <Button variant="ghost" size="sm" className="text-blue-300 hover:text-white" onClick={handleClearFilters}>
                Clear All Filters
              </Button>
            )}
            ...
            {latestCourses.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {latestCourses.map((course) => (
                  <CourseCard key={course._id} course={course} onEdit={() => handleEditCourse(course)} />
                ))}
              </div>
            ) : (
              <div className="bg-blue-900 border border-blue-800 rounded-lg p-8 text-center">
                ...
                <p className="text-blue-300 mb-4">Try adjusting your search or filter criteria</p>
                <Button
                  variant="outline"
