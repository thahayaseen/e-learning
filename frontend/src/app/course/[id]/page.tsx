...
export default function CoursePage() {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        if (params.id) {
          setLoading(true)
          const response = await getSelectedCourse(params.id as string)
          const data = response as CourseResponse
    ...
    return (
      <div className="fixed inset-0 bg-blue-950/80 flex items-center justify-center z-50">
        <div className="flex flex-col items-center bg-blue-900/50 p-8 rounded-lg shadow-lg backdrop-blur-sm">
          ...
          <p className="text-blue-100 mt-4 font-medium">Loading course details...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-950 to-blue-900">
      {courseData ? (
        <CourseBuyPage course={courseData} aldredypurchased={alreadyPurchased} />
      ) : (
        <div className="flex items-center justify-center h-screen">
          <div className="text-center p-8 bg-blue-900/50 rounded-lg shadow-lg backdrop-blur-sm">
            <h2 className="text-2xl font-bold text-blue-100 mb-2">Course Not Found</h2>
            <p className="text-blue-200 mb-4">The requested course could not be loaded.</p>
            <button
